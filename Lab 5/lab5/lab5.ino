...
Pid forwardPid(kp,ki,kd);

void forward(float travel_dist) {


  static int maxSpeedDifference = 100;

  static int minSpeed = 15;
  ...
  while ((rightCnt <= targetCount || leftCnt <= targetCount)) {
    ...
    float error = rightCnt / leftCnt;

    // Difference between counts should be 0, so setpoint is 1


    int adjustment = forwardPid.getAdjustment(rightToLeftRatio , error);
...
//    if (adjustment < -maxAdjustment) adjustment = -maxAdjustment;
    ...
    if (leftSpeed - rightSpeed > maxSpeedDifference) leftSpeed = rightSpeed + maxSpeedDifference;
    if (leftSpeed - rightSpeed < -maxSpeedDifference) leftSpeed = rightSpeed - maxSpeedDifference;
    
    rightSpeed -= adjustment;

    if (rightSpeed - leftSpeed > maxSpeedDifference) rightSpeed = leftSpeed + maxSpeedDifference;
    if (rightSpeed - leftSpeed < -maxSpeedDifference) rightSpeed = leftSpeed - maxSpeedDifference;
